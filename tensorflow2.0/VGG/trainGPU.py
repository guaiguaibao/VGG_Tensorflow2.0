...
def process_path(img_path, label):
    ...
    # 这里手动将sparse的label转换为one hot编码的label，使用ImageDataGenerator类自动转换
...
# load validation dataset，这里没有用上面定义的AUTOTUNE，而是又写了一个，是不是不能和train set共用一个，而且validation set不需要prefetch
...
def train_step(images, labels):
    with tf.GradientTape() as tape:
        ...
        # 使用低级API来训练需要传入training参数来控制dropout
...
for epoch in range(1, epochs+1):
    ...
    if test_loss.result() < best_test_loss:
        best_test_loss = test_loss.result()  # 不知道需不需要加这句，但是感觉需要
